...
- `SelectButton` wasn't localizing the new `Label` prop, fixed
- `DataTable` was leaving an empty space for sort and/or paginator even if it is
  not configured any, fixed

## v1.2.7

### Improvements

- `SelectButton` now provides `Label` prop

## v1.2.6

### Improvements

- `IHasServiceProvider` interface is introduced and implemented by
  `DelegatedInterceptor` so that value types can have access to services
...
- `ComponentDescriptor<T>` now has `Override(...)` method to allow overriding
  existing components
  - `IComponentDescriptor` and `ComponentDescriptor<T>` now allows `Type` to be
    set after its initialization
  - `ComponentDescriptor<T>` now allows `Schema` to be set after its
    initialization
...
- Remove `target` parameter and move id parameters to their own parameter name
  in action model
...
- Non-public initializers were causing rich transients not to be recognized,
  fixed
- Posting selected value as object in `Select` and `SelectButton` is now
  available via `TargetProp`
- `ISelect` component interface is provided for both `Select` and `SelectButton`
  to configure them at once through a parent configuration such as `Input`
...
- `InputText` and `Text` components were not created for nullable value types,
  fixed
