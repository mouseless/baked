...
- `DataPanel` now support toggleable option
- Context path of inputs in `SimpleForm` and `FormPage` were missing component
  names, fixed
- Max `width` calculation of `Dialog` component, fixed

## v1.1.3

### Bugfixes

- `Inputs.vue` was getting out of sync for query bound inputs, fixed

## v1.1.2

### Bugfixes

- Nhibernate, `DateOnly` cast exception, fixed.
- PrimeVue `Panel` being `display: grid` causes childs to overflow, fixed
...
- In `useDataFetcher` composable `injectedData` parameters are replaced from all
  methods with `contextData` parameters
...
- `DataTable` conventions in `DefaultThemeFeature` are now grouped and moved to
  `DataTableDefaultsUxFeature` which might cause change in behavior of your
