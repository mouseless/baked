...
- New `IgnoreOnEmpty` option added to actions. Now, when the model is `null`,
  `undefined`, or empty, actions can be skipped using the `IgnoreOnEmpty` option
...
- The method of `RemoteAction` is generated in uppercase to comply with
  standards
- The internal alignment of the `Message` component on small screens has been
  improved

### Bugfixes

- `unref` incorrectly converts blobs into empty objects, fixed
- `Action`'s fetch request, sends body on `Get` method, fixed
