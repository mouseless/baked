...
- `SimpleForm` can now hide title in inline mode without forcing it to set to
  `null`
...
- Conventions that require a level that doesn't exist on application is now
  ...
  - Not found level to default level warning is now completely removed, instead
    an info diagnostic is added
...
- `SimpleForm` submit button now shows validation messages as a tooltip, in
  dialogs
