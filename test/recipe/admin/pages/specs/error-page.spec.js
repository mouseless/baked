...
test("message from schema error infos", async({page}) => {
  ...
  await expect(errorPage.locator(baked.errorPage.message)).toHaveText("You do not have the permision to view the address or data specified.");
});
...
test("links from schema safe links", async({page}) => {
  ...
  const cardlinks = await errorPage.locator(baked.errorPage.links).locator(baked.cardLink.base).all();
