...
public class ExportSetBuilder(ExportConfiguration _configuration, IAttributePropertyBuilder _builder)
{
    ...
    {
        var attributes = BuildAttributes(type, _configuration.Type);
        ...
        typeMetadataModel.GroupName = _configuration.GetTypeGroupName(type);
    ...
    TypeExportModel BuildMembers(TypeExportModel metadata, TypeModelMembers type)
    {
        ...
        {
            var attributes = BuildAttributes(method, _configuration.Method);
        ...
        {
            var attributes = BuildAttributes(property, _configuration.Property);
            if (!attributes.Any()) { continue; }
    ...
    List<ParameterExportModel> BuildParameters(MethodModel method)
    {
        ...
        {
            var attributes = BuildAttributes(parameter, _configuration.Parameter);
            if (!attributes.Any()) { continue; }
    ...
    List<AttributeExportModel> BuildAttributes(ICustomAttributesModel model, List<IAttributeExport> attributeExports)
    {
        ...
        foreach (var attributeExport in attributeExports)
        {
            ...
            else if (!allowsMultiple && model.TryGet(attributeExport.Type, out var attribute) && attributeExport.AppliesTo(attribute, model))
            {
