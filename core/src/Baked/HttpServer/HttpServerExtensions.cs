...
public static class HttpServerExtensions
{
    ...
    {
        public void Add(string scheme, Action<AuthenticationBuilder> useBuilder,
            Func<HttpContext, bool>? handles = default
        ) => authentications.Add(new(scheme, useBuilder, handles ?? (_ => true)));
    }

    extension(IMiddlewareCollection middlewares)
    {
        public void Add<T>(
            int order = default
        ) => middlewares.Add(new(app => app.UseMiddleware<T>(), order));

        public void Add(Action<IApplicationBuilder> configure,
            int order = default
    ...
    extension(Stubber giveMe)
    {
        ...
        public HttpContext AnHttpContext(
            object[]? metadata = default,
            ClaimsPrincipal? user = default
        )
        {
    ...
    {
        public IEndpointFeature AnEndpointFeature(
            object[]? metadata = default
        )
        {
            metadata ??= [];
